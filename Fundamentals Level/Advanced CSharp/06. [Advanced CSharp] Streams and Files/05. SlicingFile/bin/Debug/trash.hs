...
selective pred functions = foldr1 (.) (filter pred functions) 

-- myStream :: Int -> [Int]
-- myStream n = (generateNums n  ++ reverse (generateNums n)) ++ myStream n
	-- where generateNums n = [x | x <- [1..n]]

-- maxFunctions functions = 

-- expandetAlphabet :: String
-- expandetAlphabet = concat ([generateAlphabet (x `mod` 27) | x <- [1..26]] ++ [generateAlphabet (x `mod` 27) | x <- [25, 24..1]]) 
	-- where generateAlphabet i = [ chr(96 + i)| rep <- [1..i]]
...
-- stream f = (\x i -> g (x i))
	...
	-- g x i
		-- | i > 5 = []
		-- | i `mod` 2 == 0 = f (x + i/2) : g x (i+1)
		-- | otherwise      = f (i**3 * x) : g x (i+1)


-- myGenerate :: Int -> [(Int -> Int)]
-- myGenerate n = [\x -> i * x | i <- [0..n]]
...
-- myNegate :: [Int] -> [Int]
-- myNegate list  = (map (\x -> abs(x) * (-1)) list)

-- myMaximum l = foldr max (head l) l

-- sumNegs :: [Int] -> Int
-- sumNegs list = foldr (+) 0 getNegatives
	-- where getNegatives = filter (\x -> x < 0) list
...
-- convertChars str = map ord str
	-- where ord Char -> Int 

-- doubleAll :: [Int] -> [Int]
-- doubleAll list = map' double list
	-- where double number = number * 2
...
-- pythagorTriples :: [(Int, Int, Int)]
-- pythagorTriples  = [(a, b, c)|c <- [2..], b <- [2..c-1], a <- [2..c-2], a*a + b*b == c*c]
